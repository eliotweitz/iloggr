...
public class UTM {
    ...
    static final long TRANMERC_NO_ERROR     =      0x0000;
    static final long TRANMERC_LAT_ERROR    =     0x0001;
    static final long TRANMERC_LON_ERROR      =    0x0002;
    static final long  TRANMERC_EASTING_ERROR  =    0x0004;
    static final long  TRANMERC_NORTHING_ERROR =    0x0008;
    static final long  TRANMERC_ORIGIN_LAT_ERROR  = 0x0010;
    static final long  TRANMERC_CENT_MER_ERROR  =   0x0020;
    static final long TRANMERC_A_ERROR        =    0x0040;
    static final long  TRANMERC_INV_F_ERROR    =    0x0080;
    static final long TRANMERC_SCALE_FACTOR_ERROR =0x0100;
    static final long  TRANMERC_LON_WARNING     =   0x0200;
	...
	public UTM(long zone, double easting, double northing, long error, char hemisphere) {
		super();
		this.zone = zone;
	 ...
	 */
	  ...
	  static long UTM_NO_ERROR    =        0x0000;
	  static long  UTM_LAT_ERROR   =        0x0001;
	  static long UTM_LON_ERROR    =       0x0002;
	  static long UTM_EASTING_ERROR   =    0x0004;
	  static long UTM_NORTHING_ERROR  =    0x0008;
	  static long UTM_ZONE_ERROR    =      0x0010;
	  static long UTM_HEMISPHERE_ERROR  =  0x0020;
	  static long UTM_ZONE_OVERRIDE_ERROR =0x0040;
	  static long UTM_A_ERROR       =      0x0080;
	  static long UTM_INV_F_ERROR   =      0x0100;
	static double PI    =       3.14159265358979323e0;  /* PI                        */
	static double MIN_LAT   =   ( (-80.5 * PI) / 180.0 );/* -80.5 degrees in radians    */
	static double MAX_LAT    =  ( (84.5 * PI) / 180.0 ) ; /* 84.5 degrees in radians     */
	static double  MIN_EASTING = 100000;
	static double MAX_EASTING = 900000;
	static double MIN_NORTHING =0;
	static double MAX_NORTHING =10000000;
	...
	public static long Set_UTM_Parameters(double a,      
			double f,
			long   override)
	{
		/*
	...
	public static UTM Convert_Geodetic_To_UTM (double Latitude,
			double Longitude) {
				/*
	 ...
	 */
	  ...
	  { /* no errors */
	    ...
	    if (Error_Code==0)
	    {
	      ...
	      result.setZone(temp_zone);
	      if (Latitude < 0)
	      {
	        False_Northing = 10000000;
	        result.setHemisphere('S');
	      }
	      else
		        result.setHemisphere('N');
	      TranMerc.Set_Transverse_Mercator_Parameters(UTM_a, UTM_f, Origin_Latitude,
	                                         Central_Meridian, False_Easting, False_Northing, Scale);
	      result = TranMerc.Convert_Geodetic_To_Transverse_Mercator(Latitude, Longitude);
	      if ((result.easting < MIN_EASTING) || (result.easting> MAX_EASTING))
	        Error_Code = UTM_EASTING_ERROR;
	      if ((result.northing < MIN_NORTHING) || (result.northing > MAX_NORTHING))
	...
	public static GeodeticCoord Convert_UTM_To_Geodetic(long   Zone,
	                             char   Hemisphere,
	                             double Easting,
	                             double Northing)
	{
	 ...
	 */
	  long Error_Code = UTM_NO_ERROR;
	  GeodeticCoord gc = new GeodeticCoord();
	  ...
	  if (Error_Code == 0) { /* no errors */
	    ...
	    gc = TranMerc.Convert_Transverse_Mercator_To_Geodetic(Easting, Northing);
...

	    {
	      ...
	      if (((error & TRANMERC_EASTING_ERROR ) != 0) || (error & TRANMERC_LON_WARNING) != 0)

	        Error_Code |= UTM_EASTING_ERROR;

	      if((error & TRANMERC_NORTHING_ERROR) != 0)
	    ...
	    if ((gc.latitude < MIN_LAT) || (gc.latitude > MAX_LAT))
	    { /* Latitude out of range */
	      Error_Code |= UTM_NORTHING_ERROR;
	    }
	  }
	   gc.setError(Error_Code);
	   return gc;
