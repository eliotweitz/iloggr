...
public class TranMerc {
	...
	static final int TRANMERC_NO_ERROR   =        0x0000;
	static final int  TRANMERC_LAT_ERROR  =       0x0001;
	static final int TRANMERC_LON_ERROR   =       0x0002;
	static final int TRANMERC_EASTING_ERROR  =    0x0004;
	static final int  TRANMERC_NORTHING_ERROR  =   0x0008;
	static final int TRANMERC_ORIGIN_LAT_ERROR =  0x0010;
	static final int TRANMERC_CENT_MER_ERROR  =   0x0020;
	static final int TRANMERC_A_ERROR       =     0x0040;
	static final int TRANMERC_INV_F_ERROR     =   0x0080;
	static final int TRANMERC_SCALE_FACTOR_ERROR =0x0100;
	static final int TRANMERC_LON_WARNING      =  0x0200;


	static final double PI =              3.14159265358979323e0;/* PI     */
	static final double PI_OVER_2   =      (PI/2.0e0);            /* PI over 2 */
	static final double MAX_LAT     =    ((PI * 89.99)/180.0);   /* 90 degrees in radians */

	static final double MAX_DELTA_LONG = ((PI * 90)/180.0);    /* 90 degrees in radians */
	static final double MIN_SCALE_FACTOR = 0.3;
	static final double MAX_SCALE_FACTOR = 3.0;
	...
	static double TranMerc_Origin_Long = -100*PI/180.0;          /* Longitude of origin in radians */
	...
	{ /* BEGIN Set_Tranverse_Mercator_Parameters */
		...
		{ /* no errors */
			...
			UTM ut = Convert_Geodetic_To_Transverse_Mercator(MAX_LAT,
					MAX_DELTA_LONG);
			TranMerc_Delta_Easting = ut.easting;
			TranMerc_Delta_Northing = ut.northing;
			ut = Convert_Geodetic_To_Transverse_Mercator(0,
					MAX_DELTA_LONG);
			TranMerc_Delta_Easting = ut.easting;
	...
	public static UTM Convert_Geodetic_To_Transverse_Mercator_Degrees (double lat, double lon) {
		return Convert_Geodetic_To_Transverse_Mercator(lat*PI/180.0, lon*PI/180.0);
	}

	public static UTM Convert_Geodetic_To_Transverse_Mercator (double Latitude,
			double Longitude)
	...
	{      /* BEGIN Convert_Geodetic_To_Transverse_Mercator */
		...
		{ /* no errors */
			...
			result.setNorthing( TranMerc_False_Northing + t1 + Math.pow(dlam,2.e0) * t2
			...
			result.setEasting( TranMerc_False_Easting + dlam * t6 + Math.pow(dlam,3.e0) * t7 
					+ Math.pow(dlam,5.e0) * t8 + Math.pow(dlam,7.e0) * t9);
		}
		result.setError(Error_Code);
		return result;
	} /* END OF Convert_Geodetic_To_Transverse_Mercator */


	public static GeodeticCoord Convert_Transverse_Mercator_To_Geodetic (
			double Easting,
			double Northing)
	{      /* BEGIN Convert_Transverse_Mercator_To_Geodetic */
		...
		GeodeticCoord coord = new GeodeticCoord();
		...
		if (Error_Code == 0)
		{
			...
			coord.latitude = ftphi - Math.pow(de,2) * t10 + Math.pow(de,4) * t11 - Math.pow(de,6) * t12 
			+ Math.pow(de,8) * t13;
			...
			/* Longitude */
			coord.setLongitude(TranMerc_Origin_Long + dlam);



			if(Math.abs(coord.latitude) > (90.0 * PI / 180.0))

				Error_Code |= TRANMERC_NORTHING_ERROR;



			if(coord.longitude > (PI))

			{

				coord.longitude -= (2 * PI);

				if(Math.abs(coord.longitude) > PI)
			...
			if (Math.abs(dlam) > (9.0 * PI / 180) * Math.cos(coord.latitude))
			...
			if(coord.latitude > 1.0e10)

				Error_Code  |= TRANMERC_LON_WARNING;
		}
		coord.error = Error_Code;
		return coord;
	...
	public static double SPHTMD(double Latitude) {
		return (TranMerc_ap * Latitude 
				- TranMerc_bp * Math.sin(2.e0 * Latitude) + TranMerc_cp * Math.sin(4.e0 * Latitude) 
				- TranMerc_dp * Math.sin(6.e0 * Latitude) + TranMerc_ep * Math.sin(8.e0 * Latitude));
	}
	...
	public static double SPHSR(double Latitude) { return (TranMerc_a * (1.e0 - TranMerc_es) / 
			Math.pow(DENOM(Latitude), 3));
	}

	public static double DENOM(double Latitude) { 
